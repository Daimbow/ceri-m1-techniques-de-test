...
errors=$(xmllint --xpath "count(//error[@severity='error'])" target/checkstyle-result.xml)
warnings=$(xmllint --xpath "count(//error[@severity='warning'])" target/checkstyle-result.xml)
...
echo $badge_url > target/checkstyle-badge-url.txt
